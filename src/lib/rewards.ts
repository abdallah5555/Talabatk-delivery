...
export type RewardRole='customer'|'driver'|'merchant';
...
const roleEventSources:Record<RewardRole,Set<string>>={
  customer:new Set(['order_delivered','referral_referrer','referral_referred','reward_redemption','admin_adjustment']),
  driver:new Set(['driver_delivery','referral_referrer','referral_referred','reward_redemption','admin_adjustment']),
  merchant:new Set(['merchant_order','referral_referrer','referral_referred','reward_redemption','admin_adjustment']),
};
...
export async function getMyRewardEvents(role:RewardRole):Promise<RewardEvent[]>{
  const {data,error}=await supabase.from('reward_events').select('id,points_delta,source,order_id,note,created_at').order('created_at',{ascending:false}).limit(100);
  if(error)throw error;
  const allowed=roleEventSources[role];
  return(data??[]).filter((row:any)=>allowed.has(String(row.source))).slice(0,80).map((row:any)=>({...row,points_delta:Number(row.points_delta)}));
}

export async function getRewardCatalog(role:RewardRole):Promise<RewardCatalogItem[]>{
  const {data:{user}}=await supabase.auth.getUser();
  if(!user)return[];
  const [{data:roleRow,error:roleError},{data,error}]=await Promise.all([
    supabase.from('user_roles').select('role').eq('user_id',user.id).eq('role',role).maybeSingle(),
    supabase.from('reward_catalog').select('id,title,description,target_role,reward_type,points_cost,reward_value,active').eq('active',true).in('target_role',[role,'all']).order('sort_order').order('created_at'),
  ]);
  if(roleError)throw roleError;if(error)throw error;
  if(!roleRow)return[];
  return(data??[]).map((x:any)=>({...x,points_cost:Number(x.points_cost),reward_value:x.reward_value==null?null:Number(x.reward_value)}));
}
