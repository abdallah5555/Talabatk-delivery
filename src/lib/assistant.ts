...
export function buildAssistantReply(raw:string,ctx:AssistantContext={}):AssistantReply{
  ...
  if(includesAny(q,['تقدر تعمل ايه','بتعمل ايه','ساعدني','مساعده','مساعدة','الاوامر','الأوامر']))return{text:'ممكن تقول مثلاً: «عايز برجر تحت 200»، «فين طلبي؟»، «عايز صيدلية»، «غير عنواني»، «افتح المفضلة»، «الطلب ناقص»، «عايز أكلم المندوب»، «نقاطي كام؟» أو «عايز أقدم كتاجر».',intent:'help'};
  ...
  if(includesAny(q,['امتي يوصل','إمتى يوصل','ميعاد الوصول','وقت الوصول','هيوصل امتى','هيوصل إمتى']))return{text:'وقت الوصول بيتغير حسب تجهيز التاجر وحالة المندوب. افتح الطلب وهتشوف تقدير التحضير وآخر حالة والتتبع.',intent:'eta',action:{type:'route',label:'فتح طلباتي',href:'/orders'}};

  if(includesAny(q,['الطلب ناقص','حاجه ناقصه','حاجة ناقصة','منتج ناقص']))return{text:'هساعدك تسجل المشكلة بسرعة. اختار الطلب واكتب المنتج الناقص، والإدارة هتتابع الشكوى من نفس الحساب.',intent:'missing_item',action:{type:'route',label:'فتح مركز المشاكل',href:'/support'}};
  if(includesAny(q,['طلب غلط','الطلب غلط','منتج غلط','جالي حاجه غلط','جالي حاجة غلط']))return{text:'سجل مشكلة «منتج/طلب غير صحيح» مع الطلب والتفاصيل، علشان تتراجع بسرعة.',intent:'wrong_item',action:{type:'route',label:'فتح مركز المشاكل',href:'/support'}};
  if(includesAny(q,['تالف','بايظ','مكسور','مشكله في المنتج','مشكلة في المنتج']))return{text:'لو المنتج تالف أو مكسور، افتح مركز المشاكل واختار الطلب واكتب وصف واضح.',intent:'damaged_item',action:{type:'route',label:'فتح مركز المشاكل',href:'/support'}};
  if(includesAny(q,['متاخر','متأخر','الطلب اتاخر','الطلب اتأخر']))return{text:'لو الطلب متأخر، راجع التتبع الأول. لو التأخير غير طبيعي افتح مشكلة مرتبطة بالطلب.',intent:'late_order',action:{type:'route',label:'فتح طلباتي',href:'/orders'}};
  if(includesAny(q,['اكلم الدعم','أكلم الدعم','خدمه العملاء','خدمة العملاء','شكوي','شكوى','مشكله','مشكلة']))return{text:'مركز حل المشاكل بيسألك عن نوع المشكلة والطلب المقصود ويحفظ المتابعة ورد الإدارة جوه حسابك.',intent:'support',action:{type:'route',label:'فتح مركز المشاكل',href:'/support'}};
  if(includesAny(q,['اكلم المندوب','أكلم المندوب','شات المندوب','رساله للمندوب','رسالة للمندوب','شات الطلب','كلم التاجر','أكلم التاجر'])){
    if(!ctx.latestOrderId)return{text:'الشات بيشتغل جوه الطلب نفسه. اعمل طلب الأول أو افتح طلب من سجل طلباتك.',intent:'driver_chat',action:{type:'route',label:'فتح طلباتي',href:'/orders'}};
    return{text:'الشات المباشر متاح لأطراف الطلب فقط، والرسائل محفوظة لحماية العميل والمندوب والتاجر.',intent:'driver_chat',action:{type:'route',label:'فتح شات آخر طلب',href:`/order-chat/${ctx.latestOrderId}`}};
  }
  ...
  if(includesAny(q,['اشعارات','إشعارات','التنبيهات','اخر التنبيهات','آخر التنبيهات']))return{text:'إشعارات الطلب دلوقتي بتفتح الطلب المرتبط بيها مباشرة.',intent:'notifications',action:{type:'route',label:'فتح الإشعارات',href:'/notifications'}};
  if(includesAny(q,['اذكار','أذكار','التذكير','ذكر']))return{text:'تذكيرات الأذكار بتلف على الأذكار المختلفة بدل تكرار ذكر واحد، وتقدر تختار من دقيقة لحد 15 دقيقة.',intent:'adhkar',action:{type:'route',label:'فتح الأذكار',href:'/adhkar'}};
  ...
  if(includesAny(q,['نقاط','نقاطي','مكافآت','مكافات','ولاء','مستواي']))return{text:'برنامج النقاط شغال: الطلبات المكتملة بتكسبك نقاط، وفي مستويات ودعوات أصحاب. افتح صفحتك علشان تشوف الرصيد والسجل.',intent:'loyalty',action:{type:'route',label:'فتح نقاطي ومكافآتي',href:'/rewards'}};
  if(includesAny(q,['دعوه','دعوة','احاله','إحالة','كود الدعوه','كود الدعوة','صاحبي','صديقي']))return{text:'من صفحة النقاط هتلاقي كود دعوتك. صاحبك يسجله قبل أول طلب مكتمل، وبعد أول طلب تاخدوا 50 نقطة لكل واحد.',intent:'referral',action:{type:'route',label:'فتح الدعوات',href:'/rewards'}};
  ...
  return{text:'فهمت إنك محتاج مساعدة، بس محتاج أعرف أكتر. ممكن تكتب اسم المنتج أو الخدمة، أو تقول «فين طلبي؟»، «عايز أغير العنوان»، «الطلب ناقص»، «نقاطي كام؟» أو «افتح الدعم».',intent:'fallback'};
}

export const ASSISTANT_QUICK_PROMPTS=['عايز حاجة أطلبها','فين طلبي؟','عايز صيدلية','عايز موبايل أو شاحن','الطلب ناقص','عايز أكلم المندوب','نقاطي كام؟','غير عنواني','عايز أقدم كتاجر'];
