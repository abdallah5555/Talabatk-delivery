create or replace function public.create_order_idempotent(p_store_id uuid,p_items jsonb,p_address text,p_request_id uuid,p_payment_method text default 'cash',p_note text default '')
returns public.orders
language plpgsql security definer set search_path=''
as $$
declare
  v_uid uuid:=auth.uid(); v_subtotal numeric(12,2); v_fee numeric(12,2); v_total numeric(12,2); v_eta integer; v_order public.orders%rowtype; v_count integer; v_redemption uuid;
begin
  ...
  select v_order.id,m.id,m.name,m.price,r.quantity from jsonb_to_recordset(p_items) r(menu_item_id uuid,quantity integer) join public.menu_items m on m.id=r.menu_item_id and m.store_id=p_store_id and m.is_available=true;
...
grant execute on function public.create_order_idempotent(uuid,jsonb,text,uuid,text,text) to authenticated;
...
declare
 ...
 gross numeric:=coalesce(NEW.delivery_fee,0); driver_platform_pct numeric:=0; fleet_driver_pct numeric:=0; fleet_platform_pct numeric:=0; merchant_pct numeric:=0;
