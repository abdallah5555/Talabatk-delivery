...
export default function Account(){
  ...
  return <ScrollView style={s.page} contentContainerStyle={s.content}>
    ...
    <Text style={s.section}>اختصاراتك</Text><View style={s.grid}><Tile icon="✨" title="مساعد طلباتك" subtitle="دور واسأل بطريقتك" onPress={()=>router.push('/assistant')}/>{hasCustomer?<Tile icon="🎁" title="مكافآت العميل" subtitle="استبدل نقاط طلباتك بجوايز للطلبات" onPress={()=>router.push({pathname:'/rewards',params:{role:'customer'}})}/>:null}{hasBusinessRole?<Tile icon="💰" title="المحفظة" subtitle="رصيد وعمولات وتسويات حساب الشغل" onPress={()=>router.push('/wallet')}/>:null}{fleet.data?.fleet_id?<Tile icon="🏢" title="شركتي" subtitle="المناديب وحسابات الشركة" onPress={()=>router.push('/fleet')}/>:null}{hasCustomer?<><Tile icon="🧾" title="طلباتي" subtitle="متابعة وسجل الطلبات" onPress={()=>router.push('/orders')}/><Tile icon="📍" title="عناويني" subtitle="بيت، شغل وأماكن محفوظة" onPress={()=>router.push('/addresses')}/><Tile icon="❤️" title="المفضلة" subtitle="أماكنك المحفوظة" onPress={()=>router.push('/favorites')}/></>:null}<Tile icon="🔔" title="الإشعارات" subtitle="آخر التحديثات" onPress={()=>router.push('/notifications')}/><Tile icon="🤲" title="الأذكار" subtitle="تذكيرات متنوعة من 1 إلى 15 دقيقة" onPress={()=>router.push('/adhkar')}/></View>

    <Text style={s.section}>الحساب والأمان</Text><Card><Menu title="ملفي الشخصي" subtitle="الصورة والاسم وإعدادات حسابك" icon="🙂" onPress={()=>router.push('/profile')}/><Divider/><Menu title="بياناتي والخصوصية" subtitle="معلومات الحساب والخصوصية" icon="👤" onPress={()=>router.push('/privacy')}/><Divider/><Menu title="الأمان وكلمة المرور" subtitle="تغيير كلمة المرور وإعدادات الحماية" icon="🔐" onPress={()=>router.push('/security')}/><Divider/><Menu title="الدعم والشكاوى" subtitle="مركز حل المشاكل ومتابعتها" icon="💬" onPress={()=>router.push('/support')}/></Card>
