...
export default function Account(){
  ...
  const name=profile.data?.full_name||'حساب طلباتك';
  return <ScrollView style={s.page} contentContainerStyle={s.content}>
    <View style={s.hero}><View style={s.profileRow}>{profile.data?.avatar_url?<Image source={{uri:profile.data.avatar_url}} style={s.avatar} contentFit="cover"/>:<View style={s.avatarPlaceholder}><Text style={s.avatarLetter}>{name[0]}</Text></View>}<View style={s.flex}><Text style={s.name}>{name}</Text><Text style={s.phone}>{profile.data?.phone||''}</Text><Text style={s.heroHint}>اختار الخدمة أو وضع الحساب اللي محتاجه.</Text></View></View></View>
    ...
    <Text style={s.section}>اختصاراتك</Text><View style={s.grid}><Tile icon="🧾" title="طلباتي" subtitle="متابعة وسجل الطلبات" onPress={()=>router.push('/orders')}/><Tile icon="📍" title="عناويني" subtitle="الحفظ والتعديل" onPress={()=>router.push('/addresses')}/><Tile icon="❤️" title="المفضلة" subtitle="أماكنك المحفوظة" onPress={()=>router.push('/favorites')}/><Tile icon="🔔" title="الإشعارات" subtitle="آخر التحديثات" onPress={()=>router.push('/notifications')}/><Tile icon="🤲" title="الأذكار" subtitle="تذكير من 1 إلى 15 دقيقة" onPress={()=>router.push('/adhkar')}/></View>

    <Text style={s.section}>الحساب والأمان</Text><Card><Menu title="بياناتي والخصوصية" subtitle="معلومات الحساب وطلبات الخصوصية" icon="👤" onPress={()=>router.push('/privacy')}/><Divider/><Menu title="الأمان والرقم السري" subtitle="إعدادات الحماية والدخول" icon="🔐" onPress={()=>router.push('/security')}/><Divider/><Menu title="الدعم والشكاوى" subtitle="مساعدة ومتابعة المشاكل" icon="💬" onPress={()=>router.push('/support')}/></Card>
