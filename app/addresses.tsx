...
import { Button, Card, Field, Muted, Screen, Title, colors } from '@/src/components/ui';
...
export default function Addresses() {
  ...
  async function add() {
    ...
    try {
      await saveAddress({label,address,latitude:point?.latitude,longitude:point?.longitude,isDefault});
      setAddress('');setPoint(null);setDefault(false);setShowMap(false);
  ...
  return <Screen>
    ...
    <Card>
      ...
      {label==='أخرى'?<Field value={label==='أخرى'?'':label} onChangeText={setLabel} placeholder="مثلاً النادي أو بيت العائلة" accessibilityLabel="اسم العنوان"/>:null}
      ...
      {showMap?<View style={s.mapWrap}><Map mapStyle={mapStyle} style={s.map} attribution logo={false} onPress={(event:any)=>{const lngLat=event.nativeEvent?.lngLat; if(lngLat){const next={longitude:Number(lngLat.longitude),latitude:Number(lngLat.latitude)};setPoint(next);}}}><Camera ref={camera} initialViewState={{center:point?[point.longitude,point.latitude]:initial,zoom:point?15:10}}/>{point?<ViewAnnotation lngLat={[point.longitude,point.latitude]}><View style={s.pin}><View style={s.pinDot}/></View></ViewAnnotation>:null}</Map><Muted>اضغط على المكان المطلوب في الخريطة لتحريك العلامة.</Muted></View>:null}
