...
import { createPosSale, getMerchantReport, getMerchantServiceAccess } from '@/src/lib/merchantReports';
...
export default function MerchantReport(){
  ...
  const cashierEnabled=access.data?.cashier_enabled!==false;
  ...
  return <Screen>
    ...
    {access.data?<Card><Muted>خطة المتجر: {access.data.merchant_plan??'free'} • الكاشير: {cashierEnabled?'مفعّل':'موقوف'}</Muted><Muted>{access.data.cashier_subscription_required?'الخدمة قابلة للاشتراك حسب إعداد المنصة.':'الكاشير مجاني حاليًا.'}</Muted></Card>:null}
    ...
    <Title>بيع يدوي / POS</Title>
    {!cashierEnabled?<Card><Text style={{textAlign:'right',fontWeight:'900'}}>خدمة الكاشير موقوفة لهذا المتجر</Text><Muted>يمكن للإدارة إعادة تفعيلها من إعدادات اشتراكات المتاجر.</Muted></Card>:<>
      <Muted>اختر الكميات. السعر والإجمالي النهائيان يُحسبان من قاعدة البيانات.</Muted>
