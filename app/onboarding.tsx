...
function MerchantForm() {
  ...
  async function pickDoc(key:MerchantDocKey,label:string){try{const asset=await chooseImage();if(!asset)return;setBusy(true);const path=await uploadOnboardingImage(asset,`merchant-${key}`,false);setDocs(v=>({...v,[key]:path}));Alert.alert('تم',`تم رفع ${label} بأمان.`);}catch(e){Alert.alert('تعذر رفع المستند',e instanceof Error?e.message:'حاول مرة أخرى');}finally{setBusy(false);}}
  ...
  async function submit(){if(!logo||!coords||!name.trim()||!category.trim()||!address.trim()||!requiredDocsComplete)return;setBusy(true);try{await submitMerchantOnboarding({businessName:name,category,address,latitude:coords.latitude,longitude:coords.longitude,logoUrl:logo,nationalIdFrontPath:docs.nationalIdFront!,nationalIdBackPath:docs.nationalIdBack!,commercialRegistrationPath:docs.commercialRegistration!,taxCardPath:docs.taxCard});Alert.alert('تم إرسال الطلب','طلب النشاط وصل للإدارة. دور التاجر لن يتفعل إلا بعد مراجعة البيانات والمستندات والموافقة.');router.replace('/pending-approval');}catch(e){Alert.alert('تعذر إرسال الطلب',e instanceof Error?e.message:'حاول مرة أخرى');}finally{setBusy(false);}}

  return <ScrollView style={s.page} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    <View style={s.hero}><Text style={s.eyebrow}>انضم كتاجر</Text><Title>خلّي نشاطك أقرب لعملائك</Title><Muted>كمّل بيانات النشاط وارفع مستندات التحقق. المستندات خاصة ولا يطلع عليها إلا صاحب الحساب والإدارة.</Muted></View>
...
function DriverForm(){
  ...
  async function submit(){if(!photo||!identityComplete||!motorcycleComplete)return;setBusy(true);try{await submitDriverOnboarding({transportMode:mode,motorcycleType,profilePhotoUrl:photo,nationalIdFrontPath:docs.nationalIdFront!,nationalIdBackPath:docs.nationalIdBack!,drivingLicenseFrontPath:docs.drivingFront,drivingLicenseBackPath:docs.drivingBack,vehicleLicenseFrontPath:docs.vehicleFront,vehicleLicenseBackPath:docs.vehicleBack,policeClearancePath:docs.policeClearance});Alert.alert('تم إرسال الطلب','الإدارة هتراجع بياناتك والمستندات. دور المندوب لن يتفعل إلا بعد الموافقة.');router.replace('/pending-approval');}catch(e){Alert.alert('تعذر إرسال الطلب',e instanceof Error?e.message:'حاول مرة أخرى');}finally{setBusy(false);}}
  return <ScrollView style={s.page} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    <View style={s.hero}><Text style={s.eyebrow}>انضم كمندوب</Text><Title>ابدأ شغلك بملف موثوق</Title><Muted>ارفع الهوية والصورة الشخصية والمستندات الخاصة بوسيلة التوصيل. المستندات خاصة بصاحب الحساب والإدارة فقط.</Muted></View>
