...
function MerchantForm() {
  ...
  async function submit(){if(!logo||!coords||!name.trim()||!category.trim()||!address.trim())return;setBusy(true);try{await submitMerchantOnboarding({businessName:name,category,address,latitude:coords.latitude,longitude:coords.longitude,logoUrl:logo});Alert.alert('تم إرسال الطلب','طلب النشاط وصل للإدارة. دور التاجر لن يتفعل إلا بعد الموافقة.');router.replace('/pending-approval');}catch(e){Alert.alert('تعذر إرسال الطلب',e instanceof Error?e.message:'حاول مرة أخرى');}finally{setBusy(false);}}
