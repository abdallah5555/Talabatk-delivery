...
function MerchantDashboard(){
  ...
  return <ScrollView style={s.page} contentContainerStyle={s.content}>
    ...
    <View style={s.two}><Action title="نقاط ومكافآت التاجر" icon="🎁" onPress={()=>router.push({pathname:'/rewards',params:{role:'merchant'}})}/><Action title="حسابي وتبديل الوضع" icon="👤" onPress={()=>router.push('/account')}/></View>
    <View style={s.two}><Action title="الأذكار" icon="🤲" onPress={()=>router.push('/adhkar')}/><Action title="المحفظة" icon="💰" onPress={()=>router.push('/wallet')}/></View>
...
function DriverDashboard(){
  ...
  const driverFee=(item:any)=>Number(item.driver_fee_snapshot??item.quoted_delivery_fee??item.delivery_fee??0);
  return <ScrollView style={s.page} contentContainerStyle={s.content}>
    ...
    <View style={s.two}><Action title="الأرباح والتقييمات" icon="💰" onPress={()=>router.push('/driver/center')}/><Action title="نقاط ومكافآت المندوب" icon="🎁" onPress={()=>router.push({pathname:'/rewards',params:{role:'driver'}})}/></View>
    <View style={s.two}><Action title="حسابي وتبديل الوضع" icon="👤" onPress={()=>router.push('/account')}/><Action title="المحفظة" icon="💳" onPress={()=>router.push('/wallet')}/></View>
    <View style={s.two}><Action title="الأذكار" icon="🤲" onPress={()=>router.push('/adhkar')}/><Action title="البلاغات والدعم" icon="💬" onPress={()=>router.push('/driver/center')}/></View>
    <Text style={s.section}>توصيلاتي الحالية</Text>
    {(active.data??[]).map(item=><Card key={item.id}><View style={s.row}><View style={s.flex}><Text style={s.cardTitle}>طلب #{item.id.slice(0,8)}</Text><Muted>{statusAr(item.status)}</Muted></View><Text style={s.orderPrice}>{driverFee(item).toFixed(0)} ج</Text></View>{item.status==='assigned'?<Button title="استلمت الطلب من التاجر" onPress={()=>advance(item.id,'picked_up')}/>:null}{item.status==='picked_up'?<Button title="أنا في الطريق للعميل" onPress={()=>advance(item.id,'on_the_way')}/>:null}{item.status==='on_the_way'?<Button title="تم التسليم" onPress={()=>advance(item.id,'delivered')}/>:null}</Card>)}
    ...
    {online&&(available.data??[]).map(item=><Card key={item.id}><View style={s.row}><View style={s.flex}><Text style={s.cardTitle}>توصيلة متاحة</Text><Muted>طلب جاهز للاستلام • حق التوصيل الظاهر للمندوب محفوظ حتى لو العميل استخدم مكافأة توصيل.</Muted></View><Text style={s.orderPrice}>{driverFee(item).toFixed(0)} ج</Text></View><Button title="قبول التوصيلة" onPress={()=>accept(item.id)}/></Card>)}
...
const s=StyleSheet.create({page:{flex:1,backgroundColor:'#f5f7fa'},content:{padding:18,paddingBottom:44,gap:13,direction:'rtl'},center:{flex:1,justifyContent:'center',backgroundColor:'#f5f7fa',padding:22,gap:10},merchantHero:{backgroundColor:'#17212f',borderRadius:30,padding:22,gap:9},driverHero:{backgroundColor:'#344054',borderRadius:30,padding:22,gap:13},driverHeroOnline:{backgroundColor:'#173b2b'},kicker:{color:'#fb923c',fontSize:12,fontWeight:'900',textAlign:'right',letterSpacing:1},heroTitle:{color:'#fff',fontSize:26,lineHeight:36,fontWeight:'900',textAlign:'right'},heroSub:{color:'#d0d5dd',fontSize:13,lineHeight:21,textAlign:'right'},statRow:{flexDirection:'row-reverse',gap:8,marginTop:5},heroStat:{flex:1,backgroundColor:'#253246',borderRadius:15,padding:11},heroStatValue:{color:'#fff',fontSize:20,fontWeight:'900',textAlign:'right'},heroStatLabel:{color:'#98a2b3',fontSize:10,textAlign:'right'},section:{fontSize:20,lineHeight:30,fontWeight:'900',color:'#101828',textAlign:'right',marginTop:5},cardTitle:{fontSize:17,lineHeight:25,fontWeight:'900',color:colors.text,textAlign:'right'},row:{flexDirection:'row-reverse',alignItems:'flex-start',gap:10},flex:{flex:1},status:{fontWeight:'900',fontSize:11},two:{flexDirection:'row-reverse',gap:8},action:{flex:1,backgroundColor:'#fff',borderWidth:1,borderColor:'#eaecf0',borderRadius:18,padding:14,gap:6,alignItems:'center'},actionIcon:{fontSize:24},actionTitle:{fontSize:12,lineHeight:19,fontWeight:'900',color:'#344054',textAlign:'center'},orderPrice:{fontSize:17,fontWeight:'900',color:colors.primary},onlineDot:{width:16,height:16,borderRadius:8,backgroundColor:'#12b76a',marginTop:6},offlineDot:{backgroundColor:'#98a2b3'},empty:{alignItems:'center',paddingVertical:30,gap:6},emptyIcon:{fontSize:34},emptyTitle:{fontWeight:'900',color:'#475467'}});
