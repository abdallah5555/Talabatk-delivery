...
function MerchantDashboard(){
  ...
  return <ScrollView style={s.page} contentContainerStyle={s.content}>
    ...
    {(stores.data??[]).map((store:any)=><Card key={store.id}><View style={s.row}><View style={s.flex}><Text style={s.cardTitle}>{store.name}</Text><Muted>{store.category} • {store.is_open?'مفتوح الآن':'مغلق'}</Muted></View><Text style={[s.status,{color:store.is_open?'#067647':'#b42318'}]}>{store.is_open?'● مفتوح':'● مغلق'}</Text></View><View style={s.two}><Action title="المنيو والمخزون" icon="🍽️" onPress={()=>router.push(`/merchant/${store.id}`)}/><Action title="الكاشير والتقارير" icon="🧾" onPress={()=>router.push({pathname:'/merchant/report/[storeId]',params:{storeId:store.id}})}/></View></Card>)}
