...
export default function Signup() {
  ...
  async function submit(){
    if(!passwordsMatch){Alert.alert('راجع كلمة المرور','كلمتا المرور غير متطابقتين.');return;}
    setBusy(true);
    try{
      await signUpPhonePassword({name,phone,password});
      if(kind==='customer') router.replace('/home');
      else router.replace({pathname:'/onboarding',params:{role:kind}});
