...
export default function Signup(){
  ...
  async function submit(){if(!passwordsMatch){Alert.alert('راجع كلمة المرور','كلمتا المرور غير متطابقتين.');return;}setBusy(true);try{await signUpPhonePassword({name,phone,password,kind});if(kind==='customer')router.replace('/home');else router.replace({pathname:'/onboarding',params:{role:kind}});}catch(e){Alert.alert('تعذر إنشاء الحساب',e instanceof Error?e.message:'راجع البيانات وحاول مرة أخرى');}finally{setBusy(false);}}
  return <ScrollView style={s.page} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    ...
    <Card><Title>بيانات الحساب</Title><Muted>{kind==='customer'?'حساب العميل يتفعل مباشرة بعد التسجيل.':'الحساب يسمح لك باستكمال البيانات فقط، والدور التشغيلي لا يتفعل إلا بعد موافقة الإدارة.'}</Muted><Field accessibilityLabel="الاسم الكامل" placeholder="الاسم الكامل" value={name} onChangeText={setName}/><Field accessibilityLabel="رقم الهاتف" keyboardType="phone-pad" placeholder="رقم الموبايل — مثال 01012345678" value={phone} onChangeText={setPhone}/><PasswordField accessibilityLabel="كلمة المرور" placeholder="كلمة المرور — 8 أحرف على الأقل" value={password} onChangeText={setPassword}/><PasswordField accessibilityLabel="تأكيد كلمة المرور" placeholder="تأكيد كلمة المرور" value={confirm} onChangeText={setConfirm}/>{confirm.length>0?<Text style={[s.match,{color:passwordsMatch?'#067647':'#b42318'}]}>{passwordsMatch?'✓ كلمة المرور متطابقة':'كلمتا المرور غير متطابقتين'}</Text>:null}<Button title={busy?'جاري إنشاء الحساب…':kind==='merchant'?'إنشاء الحساب ومتابعة بيانات النشاط':kind==='driver'?'إنشاء الحساب ومتابعة بيانات المندوب':'إنشاء حساب العميل'} onPress={submit} disabled={busy||name.trim().length<2||!phone.trim()||!passwordsMatch}/></Card>
